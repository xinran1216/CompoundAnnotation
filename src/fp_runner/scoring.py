...
def blend_score(tanimoto: float, sirius_score: float | None):
    if sirius_score is None:
        return tanimoto
    return 0.8 * tanimoto + 0.2 * sirius_score

def build_sirius_lookup(tsv: pd.DataFrame, id_col_guess=(\"id\",\"spec\",\"spectrumId\",\"spectrum_id\"), formula_col=\"molecularFormula\", score_col_guess=(\"SiriusScore\",\"score\",\"scores\" )):
    ic = None
...
def normalize_sirius_within_challenge(df: pd.DataFrame):
    df = df.copy()
